...
import { createRow, getRow, listRows, updateRow } from '$lib/server/database';
...
export const actions = {
	submitReport: async ({ request, locals }) => {
		...
		if (roundedHours > 24) {
			return {
				success: false,
				message: 'You cannot report more than 24 hours in a single report.'
			};
		}
	...
	cancelReport: async ({ request, locals }) => {
		if (!locals.user?.valid) {
			return error(403, 'You do not have permission to perform this action.');
		}
		...
		// Ensure this user is the owner of the report (or an admin)
		let report;
		try {
			report = (await getRow({
				tableId: appwriteConfig.databases.main.tables.volunteerReports,
		...
		if (report.volunteer.$id !== locals.user.id && !locals.user.admin) {
			return { success: false, message: 'You do not have permission to cancel this report.' };
		}
		// Update the report status to canceled
		await updateRow({
			tableId: appwriteConfig.databases.main.tables.volunteerReports,
			data: {
		...
		return { success: true, message: 'Volunteer report canceled successfully.' };
	}
} satisfies Actions;
